...
Four patches change Substrate ahead of upstream — egress for an actor while it resumes, without which no
skill-carrying agent of the platform boots, the atelet scheduling knobs, the keep policy on the CRD chart's
templates, and the kept podcertificate-controller namespace, without which a reinstall on the same cluster has
no working trust chain; all four are written for upstream and leave at the first release that carries them. The Postgres Secret patch is upstream's own open pull
