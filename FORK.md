...
| **v0.0.27-gs.6** (2026-09-12, annotated tag on the ledger-row commit of this release = gs.5 + #20: a worker whose pod IP changed in place is re-registered (`c3d78657`) + #21: atelet's pinned image set and the chart's `atelet.imageCache.*` (`58d24855`, `ffab110f`) + #22: lifecycle workflows outlive their caller and the router parks a request 30 s, the restore bounded and retried with `RESTORE_TIMED_OUT` (`24afbd45`, `95ce0c43`), with their ledger rows) | v0.0.26 | digests and run: recorded here once the tag's publish run has finished |
