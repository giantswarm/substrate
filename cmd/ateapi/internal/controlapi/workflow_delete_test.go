...
func TestDeleteActor_CollectsInFlightSnapshotWithoutTemplate(t *testing.T) {
	...
	w := NewActorWorkflow(persistence, nil, nil, nil, nil, nil, nil, "", nil, time.Minute, 0, objects)
...
func TestDeleteActor_CollectsSnapshotsAfterWorkerDelete(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			actorWorkflow := NewActorWorkflow(persistence, nil, nil, nil, nil, nil, nil, "", nil, time.Minute, 0, objects)
