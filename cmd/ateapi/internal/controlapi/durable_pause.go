...
// it is: it exists, and it was captured with the scope the template commits.
// A copy of another scope is not committed — a Full pause under a Data commit
// would carry memory the commit promises to drop — so the suspend uploads a
// fresh snapshot from the node, as it does for a pause without a copy.
func durablePauseCopyForCommit(actor *ateapipb.Actor, atespace string, actorTemplate *ateapipb.ActorTemplate) *ateapipb.ExternalSnapshot {
	durable := durablePauseCopy(actor)
	if durable == nil || scopeOrFull(durable.GetContentScope()) != scopeOrFull(commitSnapshotScope(atespace, actorTemplate)) {
		return nil
	}
	return durable
}

// scopeOrFull reads an unset scope as Full: the default both snapshot scopes
// take (defaultSnapshotsConfig), and what a copy recorded without a scope was
// captured with.
func scopeOrFull(scope ateapipb.SnapshotContentScope) ateapipb.SnapshotContentScope {
...
// inProgressSnapshotURI is the durable destination of a pause snapshot: the
// actor's snapshot prefix at the local snapshot's own name, so a retried
// upload re-sends the same request and overwrites the same objects.
