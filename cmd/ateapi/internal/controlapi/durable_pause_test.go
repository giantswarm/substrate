...
func TestEnsureMarkedSuspending_AdoptsDurablePauseCopy(t *testing.T) {
	...
	t.Run("paused with a durable copy", func(t *testing.T) {
		...
		if got := marked.GetStatus().GetInProgressSnapshotName(); got != "" {
			t.Errorf("InProgressSnapshotName = %q, want none: the durable copy is committed as is", got)
		}
	})
	...
	t.Run("running with a stale local snapshot record", func(t *testing.T) {
		...
		if got := marked.GetStatus().GetInProgressSnapshotName(); got == "" {
			t.Error("InProgressSnapshotName empty for a running actor, want a fresh destination")
		}
	})
}
